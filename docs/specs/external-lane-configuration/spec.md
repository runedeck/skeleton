...
#### Scenario: Lane misconfigured without a repository change

- **WHEN** a lane's dashboard state drifts from the configuration guide
- **THEN** the drift is a ceremony defect and is corrected before the next round is summoned
...
- **THEN** the correctness lane stands down, because its caller and its body each refuse fork heads before any secret-bearing step
- **AND** the ordered workflow stops after the free lanes
- **AND** the pull request merges through the owner's Repository-admin bypass once those lanes settle clean
...
### Requirement: Cursor Lane Configuration
...
### Requirement: CodeRabbit Lane Configuration
...
### Requirement: Macroscope Lane Configuration
